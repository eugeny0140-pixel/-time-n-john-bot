...
# ТОКЕН И КАНАЛЫ — ТОЛЬКО ИЗ RENDER (БЕЗОПАСНО!)
...
if not TOKEN:
    log.error("BOT_TOKEN не найден! Добавь в Render → Environment")
    exit(1)

r = Redis.from_url(REDIS_URL) if REDIS_URL else None
tr = Translator()

# 5 тестовых источников (добавим все 19 позже)
SOURCES = [
...
async def job(context):
    if CHAN1 == 0 or CHAN2 == 0:
        log.error("CHAN1/CHAN2 = 0 → проверь Environment!")
        return
    items = collect()
    if not items:
